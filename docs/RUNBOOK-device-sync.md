...
Server en `main`, verificado en vivo: `http://<IP-de-tu-Mac>:3001/api/raw` y `/api/tallas` → 200.
...
1. App → **Ajustes → Servidor → URL base** = `http://<IP-de-tu-Mac>:3001`.
...
1. Cargá **Ajustes → "Jira directo"** (baseUrl, email, apiToken con el 👁 para verificar la longitud completa, projectKey, boardIds, geminiKey — los mismos valores que en `server/.env`). **Probar conexión** → "Jira OK ✅".
